...
# processes all images in the class_x folders and puts output in rock_data_Histogram.csv
def process_dataset():
    
    labels = {"class_1": 1, "class_2": 2, "class_3": 3}
    
    with open("helperFiles/rock_data_Histogram.csv", mode = "w", newline = "") as file:
...
# trains the knn model using the 768 features in rock_data_Histogram.csv
def trainModel():
    
    global scaler, knn
    
    data = pd.read_csv("helperFiles/rock_data_Histogram.csv", header=None)
